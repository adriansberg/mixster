...
human_verification:
  ...
  - test: "Open /setup with old mixster_custom_playlists key present (no mixster_playlists). Reload — old key is gone, mixster_playlists contains migrated data with enabled:true per entry"
  ...
  - test: "Click AVSLUTT SPILL from /play, return to /setup — playlist selections are still intact"
    expected: "mixster_playlists untouched by endGame()"
...
|---|-------|--------|----------|
| 1 | User can paste a Spotify playlist URL or URI, see resolved name + track count, and have it saved across page reloads | VERIFIED | `addCustomPlaylist()` in setup page calls `/api/spotify/playlist/validate`, stores result with `enabled:true` via `savePlaylistState()` which writes to `mixster_playlists`. Persistence across reloads confirmed by `parsePlaylistState` on init restoring `customPlaylists` from that key. |
...
| `src/routes/play/+page.svelte` | Consolidated schema read, enabled-only filter, counter, double-press clear | VERIFIED | Imports parsePlaylistState+STORAGE_KEY; onMount filters enabled customs; songsPlayed increments after track assignment; clearHistory double-press; endGame leaves mixster_playlists intact |
...
| `setup/+page.svelte` | `localStorage mixster_playlists` | `savePlaylistState()` called 6 times | WIRED | Called at init, toggleDefault, toggleCustom, addCustomPlaylist, removePlaylist — every mutation path covered |
...
|-----------|--------|--------|
| STORAGE_KEY export = 'mixster_playlists' | Present at line 3 of playlist-state.ts | PASS |
...
| `mixster_custom_playlists` in setup/play pages | 0 | PASS |
| `mixster_selected_defaults` in setup/play pages | 0 | PASS |
...
2. **Old key migration**
   **Test:** Seed `mixster_custom_playlists` in localStorage via devtools (no `mixster_playlists`). Load /setup.
   **Expected:** Old key removed; `mixster_playlists` contains migrated data with `enabled:true` per entry.
